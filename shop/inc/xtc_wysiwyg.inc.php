...
function xtc_wysiwyg($type, $lang, $langID = '',$addonType='', $toolbar = 'full') 
{
	...
	if($toolbar == 'full') {
	...
	}elseif($toolbar == 'light') {
	
		//light toolbar config
		$customConfig['toolbarGroups'] ="
		toolbarGroups : [
			{ name: 'document', groups: [ 'mode', 'document', 'doctools' ] },
			{ name: 'clipboard', groups: [ 'clipboard', 'undo' ] },
		...
		//Buttons entfernen light
		$customConfig['removeButtons'] = "removeButtons: 'Save,NewPage,Print,Templates,Replace,SelectAll,Form,Checkbox,Radio,TextField,Textarea,Select,Button,ImageButton,HiddenField,Language,BidiRtl,BidiLtr,Flash,Smiley,SpecialChar,PageBreak,Iframe,ShowBlocks,About',";
    ...
    switch($type) {
        ...
        case 'products_description':
            ...
            break;
		// WYSIWYG editor products_description textarea named products_description_langID
        case 'products_description_export':
            $editorName = 'products_description_export['.$langID.']';
            $default_editor_height = 400;
            break;
			
	    // WYSIWYG editor criterion_description textarea named criterion_description[langID]
        case 'criterion_description':
            $editorName = 'criterion_description['.$langID.']';
            $default_editor_height = 100;
            break;
			
			// WYSIWYG editor products_description textarea named products_description_langID
        case 'faq':
            $editorName = 'faq_answer['.$addonType.']['.$langID.']';
