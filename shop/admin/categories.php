...
if(isset($_GET['pID']) || $_GET['action'] == 'new_product') {
function getAttributesCount() {

		$products_attributes_query = xtDBquery("select count(*) as total from ".TABLE_PRODUCTS_ATTRIBUTES." where products_id='".(int) $_GET['pID']."'");
...
if (xtc_not_null($action)) {
  switch ($action) {
	...
	case 'update_products_filter' :
		$catfunc->edit_products_filter($_POST);
		xtc_redirect(xtc_href_link(FILENAME_CATEGORIES, 'cPath='.$_GET['cPath'].'&pID='.$_GET['pID'].'&action=new_product'));
	break;
	
	case 'update_collis' :
		$catfunc->edit_collis($_POST);
		xtc_redirect(xtc_href_link(FILENAME_CATEGORIES, 'cPath='.$_GET['cPath'].'&pID='.$_GET['pID'].'&action=new_product'));
	break;
	  
	case 'update_products_images' :
		$catfunc->edit_products_images($_POST, '', 'update');
		xtc_redirect(xtc_href_link(FILENAME_CATEGORIES, 'cPath='.$_GET['cPath'].'&pID='.$_GET['pID'].'&action=new_product'));
	break;
	
	case 'update_products_text' :
		$catfunc->edit_products_text($_POST, 'update');
		xtc_redirect(xtc_href_link(FILENAME_CATEGORIES, 'cPath='.$_GET['cPath'].'&pID='.$_GET['pID'].'&action=new_product'));
	break;
...
if (USE_WYSIWYG == 'true') {
	...
	if ($action == 'edit_text' || $action == 'new_product_preview') {
	  for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
		echo xtc_wysiwyg('products_description', $data['code'], $languages[$i]['id']);
		echo xtc_wysiwyg('products_description_export', $data['code'], $languages[$i]['id']);
		echo xtc_wysiwyg('products_short_description', $data['code'], $languages[$i]['id']);
		
		//FAQ
		for ($if = 1; $if <= NUMBER_COUNT_FAQ; $if++) {
			echo xtc_wysiwyg('faq', $data['code'], $languages[$i]['id'], $if);
		}
	  }
	}
}
    ...
    <table class="tableBody">
      <tr>
        ...
        <td class="boxCenter">
              ...
              }elseif ($action == 'edit_images') {
				include (DIR_WS_MODULES.'edit_products_images.php');
			}elseif ($action == 'edit_collis') {
				include (DIR_WS_MODULES.'edit_products_collis.php');
			}elseif ($action == 'edit_filter') {
				include (DIR_WS_MODULES.'edit_products_filter.php');
			}elseif ($_GET['action'] == 'edit_text') {
				include (DIR_WS_MODULES.'edit_products_text.php');
			}elseif ($_GET['action'] == 'edit_attributes') {
				include (DIR_WS_MODULES.'edit_products_attributes.php');
			}
