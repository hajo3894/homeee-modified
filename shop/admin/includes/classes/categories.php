...
class categories {
  ...
  function remove_product($product_id) {
	...
	//export
	$product_image_export_query = xtc_db_query("SELECT products_image_export 
                                           FROM ".TABLE_PRODUCTS." 
                                          WHERE products_id = '".(int)$product_id."'");
    $product_image_export = xtc_db_fetch_array($product_image_export_query);

    $duplicate_image_export_query = xtc_db_query("SELECT count(*) AS total 
                                             FROM ".TABLE_PRODUCTS." 
                                            WHERE products_image_export = '".xtc_db_input($product_image_export['products_image_export'])."'");
    $duplicate_image_export = xtc_db_fetch_array($duplicate_image_export_query);

    if ($duplicate_image_export['total'] < 2) {
      xtc_del_image_export_file($product_image_export['products_image_export']);
    }

    //delete more images
    $mo_images_export_query = xtc_db_query("SELECT image_name 
                                       FROM ".TABLE_PRODUCTS_IMAGES_EXPORT." 
                                      WHERE products_id = '".(int)$product_id."'");
    while ($mo_images_export_values = xtc_db_fetch_array($mo_images_export_query)) {
      $duplicate_more_image_export_query = xtc_db_query("SELECT count(*) AS total 
                                                    FROM ".TABLE_PRODUCTS_IMAGES_EXPORT." 
                                                   WHERE image_name = '".xtc_db_input($mo_images_export_values['image_name'])."'");
      $duplicate_more_image_export = xtc_db_fetch_array($duplicate_more_image_export_query);
      if ($duplicate_more_image_export['total'] < 2) {
        xtc_del_image_export_file($mo_images_export_values['image_name']);
      }
    }
	
	xtc_db_query("DELETE FROM ".TABLE_SPECIALS." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_PRODUCTS_LAST_MODIFIED." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_COMPETITION_PRICES." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_EXPORTS." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_LIGHT_FEATURES." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_PRODUCTS_COLLIS." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_ATTRIBUTES_PRODUCTS_COLLIS." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_PRODUCTS_FAQ." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_SALESCHANNELS_PRICES." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_SALESCHANNELS_PRICES_HISTORY." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_SALESCHANNELS_REVENUE_PRODUCTS." WHERE products_id = '".(int)$product_id."'");
	xtc_db_query("DELETE FROM ".TABLE_SALESCHANNELS_REVENUE_ATTRIBUTES." WHERE products_id = '".(int)$product_id."'");
	...
	xtc_db_query("DELETE FROM ".TABLE_PRODUCTS_IMAGES_EXPORT." WHERE products_id = '".(int)$product_id."'");
  ...
  function insert_product($products_data, $dest_category_id, $action = 'insert') {
	...
	//not to use. see function edit_products_images
    /*
  ...
  function get_products_faq_fields($product_id, $language_id, $faq_number) {
    if (!empty($product_id)) {
      if (empty($language_id)) {
        $language_id = $_SESSION['languages_id'];
      }
      $product_query = xtc_db_query("SELECT *
                                       FROM ".TABLE_PRODUCTS_FAQ."
                                      WHERE products_id = '".(int)$product_id."'
                                        AND language_id = '".(int)$language_id."' 
									AND number = '" . (int)$faq_number . "'");
      return xtc_db_fetch_array($product_query);
    }
  }
  ...
  function edit_products_images ($products_data, $dest_category_id, $action = 'insert') {
	...
	$accepted_products_image_files_mime_types = array("image/jpeg","image/gif","image/png","image/bmp");
		...
		//prepare products_image_export filename
		if ($products_image_export = xtc_try_upload('products_image_export', DIR_FS_CATALOG_IMAGES_EXPORT_2000PX, '777', $accepted_products_image_files_extensions, $accepted_products_image_files_mime_types)) {
			
			$pname_export_arr = explode('.', $products_image_export->filename);
			$nsuffix_export = array_pop($pname_export_arr);
			$products_image_export_name = 'export_'.$products_id.'_0.'.$nsuffix_export;
			$dup_check_query = xtDBquery("SELECT COUNT(*) AS total
								                                FROM ".TABLE_PRODUCTS."
								                               WHERE products_image_export = '".$products_data['products_previous_image_export_0']."'");
			$dup_check = xtc_db_fetch_array($dup_check_query);
			if ($dup_check['total'] < 2) {
				@ xtc_del_image_export_file($products_data['products_previous_image_export_0']);
			}
			//workaround if there are v2 images mixed with v3
			$dup_check_query = xtDBquery("SELECT COUNT(*) AS total
								                                FROM ".TABLE_PRODUCTS."
								                               WHERE products_image_export = '".$products_image_export->filename."'");
			$dup_check = xtc_db_fetch_array($dup_check_query);
			if ($dup_check['total'] == 0) {
				rename(DIR_FS_CATALOG_IMAGES_EXPORT_2000PX.$products_image_export->filename, DIR_FS_CATALOG_IMAGES_EXPORT_2000PX.$products_image_export_name);
			} else {
				copy(DIR_FS_CATALOG_IMAGES_EXPORT_2000PX.$products_image_export->filename, DIR_FS_CATALOG_IMAGES_EXPORT_2000PX.$products_image_export_name);
			}
			$sql_data_array['products_image_export'] = xtc_db_prepare_input($products_image_export_name);

			//require (DIR_WS_INCLUDES.'product_images_export_1500px.php');
			require (DIR_WS_INCLUDES.'product_images_export_1000px.php');
			require (DIR_WS_INCLUDES.'product_images_export_300px.php');
			require (DIR_WS_INCLUDES.'product_images_export_220px.php');
			
			xtc_db_perform(TABLE_PRODUCTS, $sql_data_array, 'update', 'products_id = \''.xtc_db_input($products_id).'\'');

		} else {
			$products_image_export_name = $products_data['products_previous_image_export_0'];
		}
		
		//are we asked to delete some export-pics?
		if ($products_data['del_pic_export'] != '') {
			$dup_check_query = xtDBquery("SELECT COUNT(*) AS total
								                                FROM ".TABLE_PRODUCTS."
								                               WHERE products_image_export = '".$products_data['del_pic_export']."'");
			$dup_check = xtc_db_fetch_array($dup_check_query);
			if ($dup_check['total'] < 2)
				@ xtc_del_image_export_file($products_data['del_pic_export']);
		
			$messageStack->add_session(SUCCESS_FILE_DELETE_SUCCESSFULLY . ': ' . $products_data['del_pic_export'], 'success');

			xtc_db_query("UPDATE ".TABLE_PRODUCTS."
								                 SET products_image_export = ''
								               WHERE products_id    = '".xtc_db_input($products_id)."'");
		}

		if ($products_data['del_mo_pic_export'] != '') {
			foreach ($products_data['del_mo_pic_export'] AS $dummy => $val) {
				$dup_check_query = xtDBquery("SELECT COUNT(*) AS total
											                                FROM ".TABLE_PRODUCTS_IMAGES_EXPORT."
											                               WHERE image_name = '".$val."'");
				$dup_check = xtc_db_fetch_array($dup_check_query);
				if ($dup_check['total'] < 2)
					@ xtc_del_image_export_file($val);
				
				$messageStack->add_session(SUCCESS_FILE_DELETE_SUCCESSFULLY . ': ' . $val, 'success');
				
				xtc_db_query("DELETE FROM ".TABLE_PRODUCTS_IMAGES_EXPORT."
											               WHERE products_id = '".xtc_db_input($products_id)."'
											                 AND image_name  = '".$val."'");
			}
		}
		...
		//MO_PICS
		for ($img = 0; $img < MO_PICS; $img ++) {
			if ($pIMG_export = xtc_try_upload('mo_pics_export_'.$img, DIR_FS_CATALOG_IMAGES_EXPORT_2000PX, '777', $accepted_products_image_files_extensions, $accepted_products_image_files_mime_types)) {
				$pname_export_arr = explode('.', $pIMG_export->filename);
				$nsuffix_export = array_pop($pname_export_arr);
				$products_image_export_name = 'export_'.$products_id.'_'. ($img +1).'.'.$nsuffix_export;
				$dup_check_query = xtDBquery("SELECT COUNT(*) AS total
											                                FROM ".TABLE_PRODUCTS_IMAGES_EXPORT."
											                               WHERE image_name = '".$products_data['products_previous_image_export_'. ($img +1)]."'");
				$dup_check = xtc_db_fetch_array($dup_check_query);
				if ($dup_check['total'] < 2)
					@ xtc_del_image_export_file($products_data['products_previous_image_export_'. ($img +1)]);
				@ xtc_del_image_export_file($products_image_export_name);
				rename(DIR_FS_CATALOG_IMAGES_EXPORT_2000PX . $pIMG_export->filename, DIR_FS_CATALOG_IMAGES_EXPORT_2000PX . $products_image_export_name);
				//get data & write to table
				$mo_img_export = array ('products_id' => xtc_db_prepare_input($products_id), 'image_nr' => xtc_db_prepare_input($img +1), 'image_name' => xtc_db_prepare_input($products_image_export_name));
				if ($action == 'insert') {
					xtc_db_perform(TABLE_PRODUCTS_IMAGES_EXPORT, $mo_img_export);
				}
				elseif ($action == 'update' && $products_data['products_previous_image_export_'. ($img +1)]) {
					if ($products_data['del_mo_pic_export']) {
						foreach ($products_data['del_mo_pic_export'] AS $dummy => $val) {
							if ($val == $products_data['products_previous_image_export_'. ($img +1)])
								xtc_db_perform(TABLE_PRODUCTS_IMAGES_EXPORT, $mo_img_export);
							break;
						}
					}
					xtc_db_perform(TABLE_PRODUCTS_IMAGES_EXPORT, $mo_img_export, 'update', 'image_name = \''.xtc_db_input($products_data['products_previous_image_export_'. ($img +1)]).'\'');
				}
				elseif (!$products_data['products_previous_image_export_'. ($img +1)]) {
					xtc_db_perform(TABLE_PRODUCTS_IMAGES_EXPORT, $mo_img_export);
				}
				//image processing
				//require (DIR_WS_INCLUDES.'product_images_export_1500px.php');
		...
		if (isset ($products_data['products_image_export']) && xtc_not_null($products_data['products_image_export']) && ($products_data['products_image_export'] != 'none')) {
			$sql_data_array['products_image_export'] = xtc_db_prepare_input($products_data['products_image_export']);
		}
		
		//update last modified date
		$lm_data_array = array('products_id' => $products_id,
							'reason' => 'pictures',
							'notice' => '',
							'customer_id' => $_SESSION['customer_id'],
							'date' => date('Y-m-d H:i:s'));
		
		xtc_db_perform(TABLE_PRODUCTS_LAST_MODIFIED, $lm_data_array);
	
	}
	
	
	function edit_products_text($products_data, $action = 'update') {
		global $messageStack;
		...
		*/
		...
		foreach ($languages AS $lang) {
			...
			//Speichern der Eintraege fuer die FAQ - ANFANG
			for($if = 1; $if <= NUMBER_COUNT_FAQ; $if++) {
				
				$faq_question[$if] = $products_data['faq_question'][$if][$language_id];
				$faq_answer[$if] = $products_data['faq_answer'][$if][$language_id];
					
				$sql_faq_data_array[$if] = array (
									'products_id' => $products_id,
									'language_id' => $language_id,
									'number' => $if,
									'question' => $faq_question[$if],
									'answer' => $faq_answer[$if]);
									
									
				$faq_count_query[$if] = xtc_db_query('SELECT COUNT(*) as count, id FROM ' . TABLE_PRODUCTS_FAQ . ' WHERE products_id = ' . xtc_db_input($products_id) . ' AND language_id = ' . $language_id . ' AND number = ' . $if);
				$faq_count[$if] = xtc_db_fetch_array($faq_count_query[$if]);
			
				if ($faq_count[$if]['count'] > 0) {
					xtc_db_perform(TABLE_PRODUCTS_FAQ, $sql_faq_data_array[$if], 'update', 'id = '. $faq_count[$if]['id']);
				}else{
					xtc_db_perform(TABLE_PRODUCTS_FAQ, $sql_faq_data_array[$if]);  
				}
			
			}	
			//Speichern der Eintraege fuer die FAQ - ENDE
		}
	
		$messageStack->add_session(TEXT_PRODUCTS_CONTENT_SUCCESS, 'success');
	}
	
	function edit_collis($data) {
		
		global $messageStack;
		
		foreach($data as $pid => $ovid_value) {
			
			foreach($ovid_value as $ovid => $colli_data) {
				
				for($i = 1; $i <= NUMBER_COUNT_COLLIS; $i++) {
					
					save_delete_collis($pid, $ovid, $colli_data[$i], $i, $colli_data[$i]['cardboard'], $colli_data[$i]['packaging_note'], $colli_data[$i]['delete']);
		...
		//update last modified date
		$lm_data_array = array('products_id' => $pid,
							'reason' => 'collis',
							'notice' => '',
							'customer_id' => $_SESSION['customer_id'],
							'date' => date('Y-m-d H:i:s'));
		...
		$messageStack->add_session(TEXT_PRODUCTS_COLLIS_SUCCESS, 'success');
	}
	
	function edit_products_filter($filter_data) {
		
		global $messageStack;
		
		//Produkt Filter - xt-shopservice.de
		$products_id = $filter_data['products_id'];
		
		//alle moeglicherweise alten Daten entfernen
		xtc_db_query("DELETE FROM products_to_filter WHERE products_id = '".$products_id."' AND options_values_id = 0");
		
		//Daten einfuegen
		if (is_array($filter_data['filter'])) {
			foreach($filter_data['filter'] AS $filter){
				if($filter != 'none') {
					xtc_db_perform('products_to_filter',Array('products_id' => $products_id, 'filter_id' => $filter, 'options_values_id' => 0));
				}
			}
		}
		...
		$messageStack->add_session(TEXT_PRODUCTS_FILTER_EDIT_SUCCESS, 'success');
