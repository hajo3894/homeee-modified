...
require_once (DIR_FS_INC.'xtc_get_products_mo_images_export.inc.php');
?>

<?php echo xtc_draw_form('edit_products_images', FILENAME_CATEGORIES, 'cPath=' . $_GET['cPath'] . '&pID=' . $_GET['pID'] . '&action=update_products_images', 'post', 'enctype="multipart/form-data"'); ?>
 ...
 <tr>
  <td>
   <article class="breadcrumbs_dashboard">
	...
	<div class="breadcrumbs_dashboard_divider"></div>
    <a class="breadcrumbs_dashboard_current"><?php echo TEXT_PRODUCTS_DASHBOARD_EDITPICS; ?></a>
	<?php } ?>
...
<div class="dashboard_module" style="height:100%; width:350px; float:left">
<header><h3><?php echo TEXT_PRODUCTS_PICS_MAINPIC; ?></h3></header>
<div class="dashboard_module_content">
<?php
if ($pInfo->products_image_export) {
		echo '<div style="padding:15px; border-bottom:1px dotted #666666; text-align:center;">' . xtc_image(DIR_WS_CATALOG_IMAGES_EXPORT_220PX . $pInfo->products_image_export, 'Standard Image', 'width="220px" height="220px"') . '</div>';
	}else{
		echo '<div style="padding:15px; border-bottom:1px dotted #666666; text-align:center;">' . xtc_image('images/no_pic_export.jpg', 'kein Bild vorhanden') . '</div>';
	}
	echo '<div style="padding:15px; border-bottom:1px dotted #666666">' . TEXT_PRODUCTS_PICS_UPLOAD . ': ' . xtc_draw_file_field('products_image_export') . xtc_draw_hidden_field('products_previous_image_export_0', $pInfo->products_image_export) . '</div>';
	if ($pInfo->products_image_export != '') {
		echo '<div style="padding:15px">' . TEXT_DELETE . '?&nbsp;' . xtc_draw_selection_field('del_pic_export', 'checkbox', $pInfo->products_image_export) . '</div>';
	}else{
		echo '<div style="padding:15px">' . TEXT_DELETE . ' ' . TEXT_PRODUCTS_PICS_NODELETE . '</div>';
	}
?>
</div>
</div>
...
if (MO_PICS > 0) {
		$ze = 0;
		$mo_images_export = xtc_get_products_mo_images_export($pInfo->products_id);
		for ($ie = 0; $ie < MO_PICS; $ie ++) {
		$ze++;
?>
<div class="dashboard_module" style="height:100%; width:350px; float:left">
<header><h3><?php echo TEXT_PRODUCTS_PICS_PIC . ' Nr.' . ($ie +1); ?></h3></header>
<div class="dashboard_module_content">
<?php
			if ($mo_images_export[$ie]["image_name"]) {
				echo '<div style="padding:15px; border-bottom:1px dotted #666666; text-align:center">' . xtc_image(DIR_WS_CATALOG_IMAGES_EXPORT_220PX . $mo_images_export[$ie]["image_name"], 'Image ' . ($ie +1)) . '</div>';
			}else{
				echo '<div style="padding:15px; border-bottom:1px dotted #666666; text-align:center">' . xtc_image('images/no_pic_export.jpg', 'kein Bild vorhanden') . '</div>';
			}
			echo '<div style="padding:15px; border-bottom:1px dotted #666666">' . TEXT_PRODUCTS_PICS_UPLOAD . ': ' . xtc_draw_file_field('mo_pics_export_'.$ie) . xtc_draw_hidden_field('products_previous_image_export_'. ($ie +1), $mo_images_export[$ie]["image_name"]) . '</div>';
			if (isset ($mo_images_export[$ie]["image_name"])) {
				echo '<div style="padding:15px">' . TEXT_DELETE . '?&nbsp;' . xtc_draw_selection_field('del_mo_pic_export[]', 'checkbox', $mo_images_export[$ie]["image_name"]) . '</div>';
			}else{
				echo '<div style="padding:15px">' . TEXT_DELETE . ' ' . TEXT_PRODUCTS_PICS_NODELETE . '</div>';
			}
			echo '</div>';
...
<br />
<?php echo xtc_draw_hidden_field('products_id', $pInfo->products_id); ?>
