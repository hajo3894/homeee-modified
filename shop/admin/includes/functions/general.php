<?php
  ...
  */
  ...
  //deletes all product image export files by filename
function xtc_del_image_export_file($image) {
	if (file_exists(DIR_FS_CATALOG_IMAGES_EXPORT_2000PX.$image)) {
		@ unlink(DIR_FS_CATALOG_IMAGES_EXPORT_2000PX.$image);
	}
	if (file_exists(DIR_FS_CATALOG_IMAGES_EXPORT_1500PX.$image)) {
		@ unlink(DIR_FS_CATALOG_IMAGES_EXPORT_1500PX.$image);
	}
	if (file_exists(DIR_FS_CATALOG_IMAGES_EXPORT_1000PX.$image)) {
		@ unlink(DIR_FS_CATALOG_IMAGES_EXPORT_1000PX.$image);
	}
	if (file_exists(DIR_FS_CATALOG_IMAGES_EXPORT_300PX.$image)) {
		@ unlink(DIR_FS_CATALOG_IMAGES_EXPORT_300PX.$image);
	}
	if (file_exists(DIR_FS_CATALOG_IMAGES_EXPORT_220PX.$image)) {
		@ unlink(DIR_FS_CATALOG_IMAGES_EXPORT_220PX.$image);
	}
}

//deletes all product pdf files by filename
function xtc_del_pdf_file($image) {
	if (file_exists(DIR_FS_PRODUCTS_PDF.$image)) {
		@ unlink(DIR_FS_PRODUCTS_PDF.$image);
	}
}
...
function save_delete_collis($products_id, $options_values_id, $collis, $colli_nr, $cardboard, $packaging_notes, $delete = false) {
	
	//Versandart und Versandkosten und Colli-Angaben speichern
	/*
		Unbekannt = 0
		Spedition = 1
		DPD = 2
	...
	*/
	...
	if($delete) {
		...
		if($options_values_id == 0) {
			xtc_db_query('DELETE FROM ' . TABLE_PRODUCTS_COLLIS . ' WHERE products_id = ' . $products_id . ' AND colli_nr = ' . $colli_nr);
		}else{
			xtc_db_query('DELETE FROM ' . TABLE_PRODUCTS_ATTRIBUTES_COLLIS . ' WHERE products_id = ' . $products_id . ' AND options_values_id = ' . $options_values_id . ' AND colli_nr = ' . $colli_nr);
		}
		...
		$max_size_colli = 0;
		$other_size_colli = 0;
		$complete_size_colli = 0;
		
		$colli_weight = number_format((float)str_replace(',', '.', xtc_db_prepare_input($collis['weight'])), 2, '.', '');
		$colli_width  = number_format((float)str_replace(',', '.', xtc_db_prepare_input($collis['width'])), 2, '.', '');
		$colli_length = number_format((float)str_replace(',', '.', xtc_db_prepare_input($collis['length'])), 2, '.', '');
		$colli_height = number_format((float)str_replace(',', '.', xtc_db_prepare_input($collis['height'])), 2, '.', '');
		
		if($colli_weight > 0 && $colli_width > 0 && $colli_length > 0 && $colli_height > 0) {

			//Nach Gewicht beurteilen
			if ($colli_weight > 31.5) {
				$shipment_choise = 1;
			}

			//Nach Maß beurteilen
			if ($colli_width > 200 ||  $colli_length > 200 ||  $colli_height > 200) {
				$shipment_choise = 1;
			}

			if($colli_width == $colli_length && $colli_width == $colli_height) {
				$max_size_colli = $colli_width;
				$complete_size_colli = $colli_width + (2 * $colli_length) + (2 * $colli_height);
			}else{
				$max_size_colli = max(array($colli_width, $colli_length, $colli_height));

				if($max_size_colli == $colli_width) {
					$other_size_colli = (2 * $colli_length) + (2 * $colli_height);
				}
				if($max_size_colli == $colli_length) {
					$other_size_colli = (2 * $colli_width) + (2 * $colli_height);
				}
				if($max_size_colli == $colli_height) {
					$other_size_colli = (2 * $colli_length) + (2 * $colli_width);
				}
				$complete_size_colli = $max_size_colli + $other_size_colli;
			}

			if ($complete_size_colli > 360) {
				$shipment_choise = 1;
			}	


			//Paket (DPD oder DHL oder DHL-Sperrgut)
			//NEU: Upgrade-Funktion auf DHL | 2013-03-17
			if ($shipment_choise == 0) {

				if ($colli_width > 175 ||  $colli_length > 175 ||  $colli_height > 175) {
					$shipment_choise = 4;
				}else{
					if($colli_width == $colli_length && $colli_width == $colli_height) {
						$max_size_colli = $colli_width;
					...
					if($max_size_colli == $colli_width) {
						if($colli_width <= 120 && $colli_length <= 60 &&  $colli_height <= 60) {
							$shipment_choise = 3;
						}else{
							if ($complete_size_colli <= 300 && $complete_size_colli > 0) {
								if($upgrade) {
									$shipment_choise = 4;
								}else{
									$shipment_choise = 2;
					...
					if($max_size_colli == $colli_length) {
						if($colli_length <= 120 && $colli_width <= 60 &&  $colli_height <= 60) {
							$shipment_choise = 3;
						}else{
							if ($complete_size_colli <= 300 && $complete_size_colli > 0) {
								if($upgrade) {
					...
					if($max_size_colli == $colli_height) {
						if($colli_height <= 120 && $colli_length <= 60 &&  $colli_width <= 60) {
							$shipment_choise = 3;
						}else{
							if ($complete_size_colli <= 300 && $complete_size_colli > 0) {
								if($upgrade) {
			...
			if($shipment_choise == 0) {
				$shipment_choise_value = '';
			}elseif($shipment_choise == 1) {
				$shipment_choise_value = 'Spedition';
			}elseif($shipment_choise == 2) {
				$shipment_choise_value = 'dpd';
			}elseif($shipment_choise == 3) {
				$shipment_choise_value = 'dhl';
			}elseif($shipment_choise == 4) {
				$shipment_choise_value = 'dhls';
			}
			
			//packaging_notes
			if($packaging_notes['other'] != '1') {
				unset($packaging_notes['other_note']);
			}
			
			unset($packaging_notes['other']);
			

			if($options_values_id == 0) {
				
				$sql_colli_data_array = array (
							...
							'length' => $colli_length,
							'height' => $colli_height,
							'shipment_choise' => $shipment_choise_value,
							'cardboard' => $cardboard,
							'packaging_note' => implode(',', $packaging_notes),
							'upgrade' => $upgrade);
				
				xtc_db_query('DELETE FROM ' . TABLE_PRODUCTS_COLLIS . ' WHERE products_id = ' . $products_id . ' AND colli_nr = ' . $colli_nr);
				
				xtc_db_perform(TABLE_PRODUCTS_COLLIS, $sql_colli_data_array);
				...
				xtc_db_perform(TABLE_PRODUCTS_ATTRIBUTES_COLLIS, $sql_colli_data_array);
